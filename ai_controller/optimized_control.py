...
class OptimizedControl:
    ...
    def __init__(self, env, robotState, team_structure):
        ...
        previously_carrying_object = robotState.get("agents", "carrying_object", robotState.get_num_robots())
        if previously_carrying_object:
            self.held_objects = [previously_carrying_object]
        else:
            self.held_objects = []
        self.env = env
        ...
        self.last_update_message_sent = {r[0]:0 for r in robotState.get_all_robots()}
        self.trigger = False
        ...
        if self.team_structure["hierarchy"][self.env.robot_id] == "obey" and not self.team_structure["suggestions"]:
            self.time_last_suggestion_interval = float("inf")
    ...
    class Message():
        ...
        def insert(self, message, recipient, queue=False):
                ...
                if isinstance(recipient,list):
                    ...
                    if new_recipient_list:
                        if not queue:
                            ...
                            self.message_to_queue.append(message)
                            self.recipient_to_queue.append(new_recipient_list)
                else:        
                    if not queue: 
                        self.message.append(message)
                        ...
                        self.message_to_queue.append(message)
                        self.recipient_to_queue.append(recipient)
        ...
        def get(self, queue=False):
            
            if not queue:
                return_message = self.message
            else:
                return_message = self.message_to_queue
                
            return return_message
            
        def get_str(self, queue=False):
            
            if not queue:
                return_message = ''.join(self.message)
            else:
                return_message = ''.join(self.message_to_queue)
        ...
        def get_all(self, queue=False):
        
            if not queue:
                all_messages = [self.message.copy(),self.recipient.copy()]
            else:
                all_messages = [self.message_to_queue.copy(),self.recipient_to_queue.copy()]
        ...
        for rm_idx,rm in enumerate(received_messages):
                ...
                for ridx,rematch in enumerate(re.finditer(MessagePattern.item_regex_full(),new_rm[1])):
                    ...
                    if not robotState.get("agents", "type", info['robot_key_to_index'][rm[0]]) and self.planner and rm[0] in self.plan.keys() and self.plan[rm[0]] and "sense" in self.plan[rm[0]][0] and not self.planner.path_monitoring[rm[0]].init_state and self.planner.path_monitoring[rm[0]].moving_finished and not self.planner.path_monitoring[rm[0]].sensing_finished:
                        ...
                        if room_num == room: #get room location of object
                            self.planner.path_monitoring[rm[0]].update_model(time.time()-self.planner.path_monitoring[rm[0]].initial_time,"sensing", info["time"])
                        ...
                        if rematch.group(1) == self.env.robot_id:
                            if self.team_structure["hierarchy"][self.env.robot_id] == "obey" or (self.team_structure["hierarchy"][self.env.robot_id] == "order" and self.env.robot_id == rm[0]):
                                ...
                                if rematch.group(2) in info['object_key_to_index']:
                                    ...
                                    if self.order_status == self.OrderStatus.finished or self.order_similarity(rematch.group(0)):
                                        ...
                                        self.message_text.insert(MessagePattern.order_response_same(rm[0]),rm[0])
                                else:
                                    self.message_text.insert("I don't know where is object " + rematch.group(2) + ". Provide some information first. ",rm[0])
                            ...
                            if (self.team_structure["hierarchy"][self.env.robot_id] == "obey" and self.team_structure["hierarchy"][rm[0]] == "order") or (self.team_structure["hierarchy"][self.env.robot_id] == "order" and self.env.robot_id == rm[0]): 
                                if self.order_status == self.OrderStatus.finished or self.order_similarity(rematch.group(0)):
                                    ...
                                    if rematch.group(1) == self.env.robot_id:
                                        ...
                                        self.movement.help_status_info[6] = teammates_sub
                    ...
                    if (self.team_structure["hierarchy"][rm[0]] == "obey" or (self.team_structure["hierarchy"][self.env.robot_id] == "order" and self.env.robot_id == rm[0])) and not (self.commander_order_status == self.OrderStatus.giving_order and rm[0] in self.plan.keys() and self.plan[rm[0]][0] != self.previous_plan[rm[0]][0]): #If there is no human or the human is obeying #(not robotState.get("agents", "type", robot_idx) and self.team_structure["hierarchy"][rm[0]] == "obey") all(robotState.get("agents", "type", -1)) 
                        ...
                        if rm[0] in self.plan.keys() and self.plan[rm[0]]:
                            ...
                            if "sense" in self.plan[rm[0]][0]: #What if object in an other room is sensed
                                ...
                                self.planner.path_monitoring[rm[0]].update_reliability(task, success, current_time=info["time"])
                                ...
                                idx = info['object_key_to_index'][object_id]
                                weight = robotState.get("objects", "weight", idx)
                                
                                if weight > 1:
                                    if object_id in self.carry_agents:
                                        ...
                                        for c in carrying_agents:
                                            self.planner.path_monitoring[c].update_reliability("carry_heavy", success, team={0:carrying_agents}, current_time=info["time"])
                                            '''
                                            if any(humans_in_team):
                                                for human in humans_in_team:
                                                    self.planner.path_monitoring[c].update_reliability("carry_heavy_" + human, 0)
                                            else:
                                                self.planner.path_monitoring[c].update_reliability("carry_heavy", 0)
                                            '''
                                else:
                                    task = self.planner.node_type[self.plan[rm[0]][0]]
                                    self.planner.path_monitoring[rm[0]].update_reliability(task, success, current_time=info["time"])
            ...
            if not template_match and not robotState.get("agents", "type", info['robot_key_to_index'][rm[0]]): #Human sent a message, we need to translate it. We put this condition at the end so that humans can also send messages that conform to the templates
                ...
                if asking_for_help:
                    ...
                    if functions and not (obeying and not self.team_structure["suggestions"]):
                        ...
                        if "hierarchy" in self.team_structure:
                            ...
                            if self.team_structure["hierarchy"][self.env.robot_id] == "order" and not all(plan_similarities): 
                                ...
                                for sp in self.plan.keys():
                                    if 'carry' in self.plan[sp][0] and not self.agent_requesting_order[sp] and sp != rm[0]:
                        ...
                        if not all(plan_similarities) and (not (obeying and self.env.robot_id not in functions.keys()) or (obeying and not time_limit_suggestion)):
                            ...
                            if self.proposed_plan:
                                try:
                                    ...
                                    for plan in self.proposed_plan:
                                        agent_id = plan[0]
                                        agent_plan[agent_id] = plan[1][0][0]
                                        
                                    for agent_key in functions.keys():
                                        if agent_key in agent_plan.keys() and functions[agent_key][0] == agent_plan[agent_key]:
                                            proposed_plan_similarities.append(True)
                                        else:
                                            proposed_plan_similarities.append(False)
                                   
                                    print("Previous time interval: ", self.time_last_suggestion_interval)
                                    ...
                                    if "sense" in agent_function or "carry" in agent_function:
                                        ...
                                        if "sense" in agent_function:
                                            ...
                                            if not ("ROOM" in agent_function and agent_function.split("ROOM")[1] not in self.planner.rooms.keys()) and (rm[0],new_ob_room) not in self.planner.sensed_clusters and agent_function in self.planner.nodes:
                                                ...
                                                elif ob_room.split("ROOM")[1] in self.all_rooms.keys():
                                                    explanation = "Room " + ob_room.split("ROOM")[1] + " is empty. "
                                                else:
                                            ...
                                            if "leader" in agent_function: #TODO
                                                ...
                                                if not ob_location or (ob_location[0] == -1 and ob_location[1] == -1):
                                                    ...
                                                    explanation = "We still don't have enough certainty over object " + object_id + ", so far it is " + str(round(self.planner.belief[object_id]*100,2)) + "% of being dangerous. We still need some more measurements to be sure. "
                                        ...
                                        if not feasible_action:
                                            ...
                                            for sk_idx in range(len(skip_states2)): 
                                                if skip_states2[sk_idx][1] == agent_key:
                                                    del skip_states2[sk_idx]
                                                    break
                            ...
                            if not (obeying and time_limit_suggestion):
                                ...
                                if obeying:
                                    self.proposed_plan,proposed_objectives = self.planner.replan([], pretend=True, current_time=info["time"])
                                    plan_description = self.get_plan_description(self.proposed_plan)
                                ...
                                if any_feasible_action:
                                    ...
                                    plan2,objectives = self.planner.replan(skip_states2, pretend=True, current_time=info["time"])
                                    ...
                                    if plan2:
                                        ...
                                        ignored_objectives = ['completeness','agent utilization','reliability']
                                        for objc_idx in range(len(objectives[0])):
                                        
                                            if (obeying or self.team_structure["hierarchy"][self.env.robot_id] == "order") and objectives[2][objc_idx] not in ignored_objectives:
                                                ...
                                                elif name == "object carrying":
                                                    if overall_per_objective[-1]:
                                                        explanation = "There is an increase in the number of dangerous objects disposed. "
                                                    else:
                                                        explanation = "There is a decrease in the number of dangerous objects disposed. "
                                                        
                                                objective_explanation += explanation
                                                    
                                        
                                        if not overall_objective:
                                            pdb.set_trace()
                                        ...
                                        if not obeying:
                                            explanation_queue = objective_explanation + "Your plan doesn't seem feasible. Let's stick with the previous plan. "
                                            objective_explanation = MessagePattern.plan_evaluation_bad() + "[" + final_function_descriptions + "]. "
                                    ...
                                    if obeying and obeying_present:
                                        
                                        change_team = False
                                        same_function = False
                                        if self.env.robot_id in self.plan and self.plan[self.env.robot_id] and self.plan[self.env.robot_id][0] == functions[self.env.robot_id][0]:
                                            same_function = True
                                            if "carry" in functions[self.env.robot_id][0]:
                                                change_team = self.get_teammates_from_plan(functions, skip_states2, robotState, info)
                          
                                        
                                        if not same_function or change_team:    #missing self.order_status = self.OrderStatus.ongoing                             
                                            self.movement.cancel_cooperation(self.State.decision_state,self.message_text)
                                            
                                            if robotState.object_held and not change_team:
                                                self.create_action_function("drop()")
                                            else:
                                                self.action_function = ""
                                            ...
                                            self.plan = functions
                                            print("I'm here in commands_to_functions 1")
                            
                                elif obeying and not obeying_present:
                                    explanation_queue = objective_explanation
                                    objective_explanation = MessagePattern.plan_evaluation_bad() + "[" + final_function_descriptions + "]. "
                                else:
                                    ...
                                    if not obeying:
                                        ...
                                        explanation_queue = objective_explanation + "I cannot follow your orders. "
                                        objective_explanation = MessagePattern.plan_evaluation_bad() + "[" + final_function_descriptions + "]. "
                                        
                                    
                                
                                
                            elif obeying and self.env.robot_id in functions.keys():
                                ...
                                if not same_function or change_team: 
                                    self.movement.cancel_cooperation(self.State.decision_state,self.message_text)
                                    if robotState.object_held and not change_team:
                                        self.create_action_function("drop()")
                                    else:
                                    ...
                                    self.functions_to_execute = self.commands_to_functions(functions, skip_states2, rm[0], robotState, info)
                                    self.plan = functions
                                    print("I'm here in commands_to_functions 2")
                        ...
                        if explanation_queue:
                            self.message_text.insert(explanation_queue,rm[0],True)
                
                    elif not translated_message and (obeying and not self.team_structure["suggestions"]):
                    
                        translated_message = self.human_to_ai_text.noop
    ...
    def order_similarity(self, proposed_order):
    
        different_order = True
    
        if re.search(MessagePattern.order_collect_group_regex(),proposed_order):
            rematch = re.search(MessagePattern.order_collect_group_regex(),proposed_order)
            teammates = rematch.group(2)[1:].replace("]", "").split(",")
            
            if "collect_object" in self.action_function and self.movement.help_status_info[0]:
                
                first_argument = self.action_function.split(",")[0]
                object_collect = eval(first_argument[first_argument.index("(")+1:])
                
                if set(teammates) == set(self.movement.help_status_info[0]) and object_collect == rematch.group(4):
                    different_order = False
                
            elif self.action_index == self.movement.State.follow:
                ...
                if rematch.group(1) != self.env.robot_id:
                    ...
                    if self.movement.help_status_info[0][0] == rematch.group(1):
                        different_order = False
                    
                    if not different_order and set(teammates_sub) != set(self.movement.help_status_info[6]):
                        self.movement.help_status_info[6] = teammates_sub
            
        
        elif re.search(MessagePattern.order_sense_room_regex(),proposed_order):
            rematch = re.search(MessagePattern.order_sense_room_regex(),proposed_order)
            if "sense_room" in self.action_function:
                first_argument = self.action_function.split(",")[0]
                room = eval(first_argument[first_argument.index("(")+1:])
                if room == rematch.group(2):
                    different_order = False
        elif re.search(MessagePattern.order_collect_regex(),proposed_order):
            rematch = re.search(MessagePattern.order_collect_regex(),proposed_order)
            if "collect_object" in self.action_function and not self.movement.help_status_info[0]:
                first_argument = self.action_function.split(",")[0]
                object_collect = eval(first_argument[first_argument.index("(")+1:])
                if object_collect == rematch.group(2):
                    different_order = False
        elif re.search(MessagePattern.order_sense_regex(),proposed_order):
            pass
        ...
        if "carry" in command:
            ...
            if object_weight > 1 or (not object_weight and leader and leader != self.env.robot_id):
                if leader:
                    ...
                    if leader == self.env.robot_id:
                        if len(teammates)+1 >= object_weight:
                            ...
                            self.message_text.insert(MessagePattern.order_response(sender, "collect") + "I will lead this operation with the support of [" + ",".join(teammates) + "]. ",sender)
                ...
                if object_id in info['object_key_to_index'].keys():
                    ...
                    if leader == self.env.robot_id:
                        self.message_text.insert("I don't know what is the weight of this object, I'll just carry it myself. ",sender)
    ...
    def get_teammates_from_plan(self,functions, keys_leaders, robotState, info):
        ...
        if self.movement.help_status == self.movement.HelpState.being_helped:
            if leader != self.env.robot_id:
                change_team = True
            elif set(self.movement.help_status_info[0]) != set(teammates) and not set(self.movement.help_status_info[0]).issubset(set(teammates)):
                change_team = True
        elif self.movement.help_status == self.movement.HelpState.helping and leader != self.movement.help_status_info[0][0]:
            change_team = True
    ...
    def check_carrying_agents(self,carrying_object,robot_id,robotState,info):
        already_carrying = ""
        agent_carrying = ''
        
        if carrying_object:        
            already_carrying = carrying_object
            agent_carrying = robot_id
        else:
            for r_id in self.plan.keys():
                if robot_id != r_id and self.plan[r_id] and self.plan[robot_id][0] == self.plan[r_id][0]:
                    other_carrying_object = robotState.get("agents", "carrying_object", info['robot_key_to_index'][r_id])
                    if other_carrying_object:
                        already_carrying = other_carrying_object
                        agent_carrying = r_id
                        break
        
        
        object_to_be_carried = self.plan[robot_id][0].split('_')[1]
        
        print("objects to carry", object_to_be_carried, already_carrying, agent_carrying)
        if object_to_be_carried == already_carrying:
        ...
        elif already_carrying and robot_id == agent_carrying and self.planner.path_monitoring[robot_id].object_carried != already_carrying:
            self.message_text.insert("You are carrying the wrong object, " + agent_carrying + "! You should carry object " + object_to_be_carried + ". ",agent_carrying)
            self.planner.path_monitoring[robot_id].object_carried = already_carrying
            team_agents = [r_id for r_id in self.plan.keys() if self.plan[r_id] and self.plan[robot_id][0] == self.plan[r_id][0]]
            
            if len(team_agents) == 1:
                self.planner.path_monitoring[robot_id].update_reliability("carry", 0, current_time=info["time"])
            else:
                for c in team_agents:
                    self.planner.path_monitoring[c].update_reliability("carry_heavy", 0, team={0:team_agents}, current_time=info["time"])
    ...
    def identify_room(self, robotState, info):
        for idx in range(len(self.other_agents)):
            ...
            carrying_object = robotState.get("agents", "carrying_object", idx)
            
            if carrying_object:
                print("CARRYING OBJECT:", carrying_object)
            
            if robot_id in self.plan and self.plan[robot_id] and "carry" in self.plan[robot_id][0] and self.planner and robot_id in self.planner.path_monitoring.keys() and self.planner.path_monitoring[robot_id].goal != "goal area":
                self.check_carrying_agents(carrying_object,robot_id,robotState,info)
            ...
            if robot_location_known and ((room_num and not self.other_agents[idx].rooms) or (self.other_agents[idx].rooms and room_num != self.other_agents[idx].rooms[-1])):
                ...
                if self.planner and robot_id in self.planner.path_monitoring.keys() and self.planner.path_monitoring[robot_id] and not self.planner.path_monitoring[robot_id].init_state:
                    if not self.planner.path_monitoring[robot_id].current_node:
                        try:
                            if len(self.other_agents[idx].rooms) > 1:
                                self.planner.path_monitoring[robot_id].set_current_node(self.other_agents[idx].rooms[-2])
                            else:
                                self.planner.path_monitoring[robot_id].set_current_node(self.other_agents[idx].rooms[-1])
                        except:
                            pdb.set_trace()
                            
                    
                    if robot_id in self.plan and self.plan[robot_id] and "carry" in self.plan[robot_id][0] and self.planner.path_monitoring[robot_id].goal != "goal area":        
                        self.check_carrying_agents(carrying_object,robot_id,robotState,info)
                        ...
                        if self.planner.path_monitoring[robot_id].consecutive_increases - self.planner.path_monitoring[robot_id].consecutive_increases_limit >= sanction_limit:
                            try:
                                if robot_id in self.plan.keys():
                                    ...
                                    if self.planner.node_type[self.plan[robot_id][0]] == 'carry_heavy':
                                
                                        carrying_agents = [s for s in self.plan.keys() if self.plan[s] and self.plan[s][0] == self.plan[robot_id][0]]
                                        for c in carrying_agents:
                                            self.planner.path_monitoring[c].update_reliability("carry_heavy", 0, team={0:carrying_agents}, current_time=info["time"])
                                    else:       
                                        self.planner.path_monitoring[robot_id].update_reliability(self.planner.node_type[self.plan[robot_id][0]], 0, current_time=info["time"])
                                elif robot_id in self.legacy_plan.keys():
                                    if self.planner.node_type[self.legacy_plan[robot_id][0]] == 'carry_heavy':
                                
                                        carrying_agents = [s for s in self.legacy_plan.keys() if self.legacy_plan[s] and self.legacy_plan[s][0] == self.legacy_plan[robot_id][0]]
                                        for c in carrying_agents:
                                            self.planner.path_monitoring[c].update_reliability("carry_heavy", 0, team={0:carrying_agents}, current_time=info["time"])
                                    else:       
                                        self.planner.path_monitoring[robot_id].update_reliability(self.planner.node_type[self.legacy_plan[robot_id][0]], 0, current_time=info["time"])
                            except:
                                pdb.set_trace()
                            
                    elif moving_result == 2:
                        if robot_id in self.plan and self.plan[robot_id] and "carry" in self.plan[robot_id][0]: #maybe wait until object is carried
                            self.check_carrying_agents(carrying_object,robot_id,robotState,info)
    ...
    def control(self,messages, robotState, info, next_observation):
        ...
        if robotState.dropped_objects:
            ...
            print("dropped objects", robotState.dropped_objects)
            ...
            for od in robotState.dropped_objects:
                ...
                for agent_od in od:
                    ...
                    if agent_od[0] != self.env.robot_id:
                        ...
                        agent_type = robotState.get("agents", "type", agent_idx)
                        if not agent_type:
                            humans_in_team[agent_od[1]].append(agent_od[0])
                    '''
            for od in robotState.dropped_objects:
                for agent_od in od:
                    ...
                    if any(humans_in_team[agent_od[1]]):
                        for human in humans_in_team[agent_od[1]]:
                            self.planner.path_monitoring[agent_od[0]].update_reliability("carry_heavy_" + human, 0)
                    else:
                ...
                if not self.action_function or self.help_requests:
                    ...
                    if function_str:
                        ...
                        if external_function:
                            self.message_text.insert(robotState.current_action_description,str(self.leader_id))
    ...
    def send_message(self,message, robotState, next_observation, info):
        ...
        messages = self.get_all_messages(message, False, robotState, next_observation, info)
        messages_queue = self.get_all_messages(message, True, robotState, next_observation, info)

        action["message"] = messages
        action["message_queue"] = messages_queue
        #action["message_ai"] = message_ai

        return action,finished,output
    
    def get_all_messages(self,message, queue, robotState, next_observation, info):
        ...
        message_str = message.get_str(queue=queue)
        
        if message_str:
        
            messages.append(message_str)
            
            all_messages = message.get_all(queue=queue)
    ...
    def decision_obey(self,messages, robotState, info, output, nearby_other_agents, next_observation):
        ...
        if robotState.object_held and "collect" not in self.action_function:
            function_output = "drop()"
        else:
        ...
        if self.order_status == self.OrderStatus.reporting_output:
            if self.order_status_info and not self.team_structure["hierarchy"][self.env.robot_id] == "order":
                if "sense_object" in self.order_status_info[0] or "sense_room" in self.order_status_info[0]:
                    ...
                    try:
                        print('info PROVIDED:', self.other_agents[info['robot_key_to_index'][str(self.leader_id)]].items_info_provided, self.order_status_info[1], self.leader_id) #, self.other_agents[info['robot_key_to_index'][str(self.leader_id)]].items_info_provided)
                    except:
                        pdb.set_trace()
                    objects_reported = []
                    for ob in self.order_status_info[1]:
    ...
    def update_planner(self, robotState, skip_states, objects_to_carry=[]):
        ...
        for ag in robotState.get_all_robots():
            ...
            if ag[0] != self.env.robot_id:
                ...
                ego_location = np.where(robotState.latest_map == 5) 
                ag_location = [int(ego_location[0][0]),int(ego_location[1][0])]
                
            agents_initial_positions[ag[0]] = list(ag_location)
        ...
        if not self.planner:
            ...
            log_name = ''            
            if robotState.args.log:
                log_name = self.env.log_name
            
            self.planner = DynamicSensorPlanner(self.env.robot_id, agents, objects, estimates, locations, agents_initial_positions, pd_pb, object_weights, occMap, self.extended_goal_coords, self.new_rooms, agents_type, objects_to_carry, log_name=log_name)
            if robotState.args.no_reset:
                rooms = robotState.get_all_object_rooms()
                for r in rooms:
                    room_replace = r.replace("room ", "")
                    if room_replace in self.all_rooms.keys() and room_replace not in self.new_rooms.keys():
                        self.new_rooms[room_replace] = self.all_rooms[room_replace]
    ...
    def decision(self,messages, robotState, info, output, nearby_other_agents, help_requests):
        ...
        if (self.commander_order_status == self.OrderStatus.cancelling_order and not self.providing_plan) or not self.plan:
            ...
            self.update_planner(robotState,skip_states)
            if not self.plan:
                plan,self.optimization_metrics = self.planner.replan(current_time=info["time"])
                ...
                plan,self.optimization_metrics = self.planner.replan(skip_states, current_time=info["time"])
                
                #FINISH
                if not plan and not self.planner.nodes:#Sure? TODO
                    self.message_text.insert(MessagePattern.finish(),"all")
            ...
            print("Changing plan according to user", plan)
        ...
        if self.commander_order_status == self.OrderStatus.cancelling_order:
            ...
            if not plan:
                self.message_text.insert("I couldn't generate a plan, I will try to think harder if you ask me again! ","all")
            ...
            for agent_id in self.plan.keys():
                ...
                if not self.plan[agent_id]:
                    ...
                    print("deleting", agent_id)
                    continue
                
                print("truth", agent_id not in self.previous_plan or 'sense' in self.plan[agent_id][0] or self.plan[agent_id][0] != self.previous_plan[agent_id][0] or ('carry' in self.plan[agent_id][0] and weight > 1))
            ...
            for ob in changed_carry:
                ...
                to_delete = []
                for robot_id in self.carry_agents[ob]:
                    if robot_id not in self.plan.keys():
                        to_delete.append(robot_id)
                for agent_id in to_delete:
                    self.carry_agents[ob].remove(agent_id)
                    self.message_text.insert(MessagePattern.order_cancel(agent_id),agent_id)
                    print("who knows where " + agent_id + " is")
