...
class ChaseBall(Controller):
    ...
    def __init__(self, port: int = 1071, check_version: bool = True, launch_build: bool = True):
        ...
        self.person = FirstPersonAvatar(avatar_id="b", position={"x": -1.4, "y": 0, "z": 0}, look_speed = 200, move_speed= 2)

 

        self.add_ons.extend([self.robot, self.magnebot, self.object_manager, self.camera, self.person]) #image_capture, self.person])
    ...
    def run(self):
        ...
        levitate_started = False



        while not done:
            if self.change_position_called:
                new_pos = TDWUtils.array_to_vector3(self.person.transform.position + [0.1,0,0])
                commands.extend([{"$type": "move_avatar_towards_position", "position": new_pos, "speed": 0.1, "avatar_id": "b"}])
                self.change_position_called = False
                print(new_pos)
            ...
            if self.person.left_button_pressed:
                print(self.magnebot.dynamic.held[Arm.right], self.ball_id)
...
{"$type": "apply_force_magnitude_to_object", "magnitude": 1,"id": self.ball_id2}])
                    '''
                    if not levitate_started:
                        commands.extend([{"$type": "add_constant_force", "force": {"x": 0, "y": 60, "z": 0}, "id": self.ball_id2}]) #59
                        levitate_started = True
                    else:
                        commands.extend([{"$type": "add_constant_force", "force": {"x": 0, "y": 0, "z": 0}, "id": self.ball_id2}])
                        levitate_started = False

                if np.linalg.norm(self.object_manager.transforms[self.ball_id].position - self.person.transform.position) < 3:
                    print('tata')
                    #commands.extend([{"$type": "set_mass", "mass": 99, "id": self.ball_id}])
                    commands.extend([{"$type": "apply_force_to_object", "force": {"x": 0, "y": 10, "z": 0}, "id": self.ball_id}])
                    print('tata2')

            
            if levitate_started:
                if self.object_manager.transforms[self.ball_id2].position[1] > 1:
                    force = 2-(self.object_manager.transforms[self.ball_id2].position[1]-1)
                    commands.extend([{"$type": "add_constant_force", "force": {"x": 0, "y": 55, "z": 0}, "id": self.ball_id2}])
                
                elif self.object_manager.transforms[self.ball_id2].position[1] < 1:
                    force = 2-self.object_manager.transforms[self.ball_id2].position[1]
                    commands.extend([{"$type": "add_constant_force", "force": {"x": 0, "y": 60, "z": 0}, "id": self.ball_id2}])
