...
class ChaseBall(Controller):
    ...
    def __init__(self, port: int = 1071, check_version: bool = True, launch_build: bool = True):
        ...
        self.person = FirstPersonAvatar(avatar_id="b", position={"x": -1.4, "y": 0, "z": 0})
        self.person2 = FirstPersonAvatar(avatar_id="c")
        image_capture = ImageCapture(avatar_ids=["b"], path="")
        image_capture.set(avatar_ids=["b"],save=False)
        self.keyboard = Keyboard()
        self.keyboard.listen(key="Escape", function=self.change_position)
        self.add_ons.extend([self.robot, self.magnebot, self.object_manager, self.camera, self.person, self.keyboard, self.person2, image_capture]) #image_capture, self.person])
    ...
    def run(self):
        ...
        im1 = ax.imshow( np.zeros( ( 256, 256, 3 ) ) ) # Blank starting image
        fig.show()
